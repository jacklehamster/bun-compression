...
export default class Compressor {
     ...
     */
    async loadAndCompress(files: string[], fetcher?: IFetcher<string>, encoder: EncoderEnum[] = DEFAULT): Promise<ArrayBufferLike> {
     ...
     * @param files files to load.
     */
    compress(data: Record<string, any>, encoder: EncoderEnum[] = DEFAULT): ArrayBufferLike {
        const tokenizer = new Tokenizer();
    ...
    expand(arrayBuffer: ArrayBufferLike, config?: ExtractionConfig): ExtractableData {
        return new ExtractableData(this.expandDataStore(arrayBuffer), config);
    }

    private compressDataStore(dataStore: DataStore, encoderEnums: EncoderEnum[] = DEFAULT): ArrayBufferLike {
        ...
        this.streamDataView(headerBuffer, finalStream);
        ...
        for (let index = 0; index < dataStore.files.length; index++) {
            ...
            this.streamDataView(subBuffer, finalStream);
        }
    ...
    private expandDataStore(arrayBuffer: ArrayBufferLike): DataStore {
        const compressedSize = arrayBuffer.byteLength;
        let input = arrayBuffer;
        const globalStream = this.streamDataView(input);
        ...
        const headerTokenEncoder = new TokenEncoder(this.streamDataView(headerBuffer));
        const headerTokens = headerTokenEncoder.decodeTokens(true);
        const files = headerTokenEncoder.decodeNumberArray();

        const subBuffers: ArrayBufferLike[] = [];
        do {
        ...
        const getDataTokens = (index: number) => {
            ...
            const streamDataView = this.streamDataView(subBuffer);
    ...
    private applyEncoders(buffer: ArrayBufferLike, encoders: Encoder[]): ArrayBufferLike {
    ...
    private applyDecoders(buffer: ArrayBufferLike, decoders: Encoder[]): ArrayBufferLike {
        let resultBuffer = buffer;
    ...
    private streamDataView(buffer: ArrayBufferLike, stream?: StreamDataView) {
        if (stream) {
            stream.setNextBytes(buffer as ArrayBuffer);
            return stream;
        }
        return new StreamDataView(buffer as ArrayBuffer);
    }
}
